...
int showfinal = 0;
int draw = 0;
const int Selection = 0;//choose nGd or nH or Unified
...
void profile_minimizer(ROOT::Math::Minimizer *mini){
    ...
    for(int s = 0;s<set;s++){
        ...
        if(s==0)
            ...
            errorBudget[s] =pow(array_sin2_2theta13[s][1],2)-pow(array_sin2_2theta13[s-1][1],2);
...
void DrawOscillationCurve(ROOT::Math::Minimizer* mini){
    ...
    c1->SaveAs("./Figures/nGd/effectiveCurve.png");
    ...
    data[2]->SetTitle("Oscillation results;            Effective baseline L_{eff}(meter);R^{obs}/R^{pred}_{no_osc}");
    ...
    lgOsc->Draw("SAME");
    c2->SaveAs("./Figures/nGd/oscillationCurve.png");
...
void DrawContour(ROOT::Math::Minimizer* mini){
    ...
    gr2d[2]->SetTitle("Confidence level contour;\\sin^{2}2\\theta_{13};A_{norm}");
    ...
    gr2d[2]->GetXaxis()->SetLimits(0.07,0.1);
    gr2d[2]->GetYaxis()->SetLimits(0.88,1.04);
    gr2d[2]->Draw("alf");
    gr2d[1]->Draw("lfSAME");
    gr2d[0]->Draw("lfSAME");
    c1->cd();
    ...
    scanChi2(mini,104,npoints,s2_2th13,dChi,0.07,0.1);
    scanChi2(mini,105,npoints,A,dChi2,0.88,1.04);
    ...
    pad2->cd();
    pad2->SetGrid();
    gr1d[0]->GetXaxis()->SetLimits(0.07,0.1);
    ...
    gr1d[1]->GetYaxis()->SetLimits(0.88,1.04);
    ...
    c1->cd();
    pad3->Draw("SAME");
    c1->SaveAs("./Figures/nGd/contour.png");
