...
void reinitialize_minimizer(ROOT::Math::Minimizer *mini);
...
double fcn(const double* par){
    ...
    for(int r = 0;r<N_reactors;r++){
        double sum = 0;
        for(int n = 0;n<N_nuclears;n++){
            fraction[r][n] = Fraction[n]+par[fracCount++];        
            sum+=fraction[r][n];
        }
        for(int n = 0;n<N_nuclears;n++){
            fraction[r][n]/=sum;
        ...
        for(int bin = 0;bin<energy_bins;bin++){
            ...
            for(int r = 0;r<N_reactors;r++){
                ...
                if(norm)
                    ...
                    Expected[d]+=ReactorShape[r][d][bin]*np[d]*_cross_section[bin]*1.e-4/(4*3.14*dist_map[d][r]*dist_map[d][r])*86400*eps[d]*_prob;//0.80 is efficiency
            }
        }
        if(debug)
    ...
    for(int r = 0;r<6;r++){
        for(int n1 = 0;n1<4;n1++){
            for(int n2 = 0;n2<4;n2++){
                chi2+=corrSingleChi2(par[fracPullIndexBegin+r*4+n1],0,par[fracPullIndexBegin+r*4+n2],0,SystActinideFraction,abs(invertCorrArray[n1*4+n2]));
                if(debug)
...
void reinitialize_minimizer(ROOT::Math::Minimizer *mini){
    ...
    for(int r = 0;r<6;r++){
        parname = TString::Format("Pull_Power_%i",r);
       mini->SetFixedVariable(idx++,parname,0);  
    }
    mini->SetFixedVariable(idx++,"Pull_ReleaseEnergyPerFission",0);
    for(int r = 0;r<6;r++)
        for(int d = 0;d<4;d++){
            parname = TString::Format("Pull_Fraction_Reactor_Nuclear_%i_%i",r,d);
            mini->SetFixedVariable(idx++,parname,0);
    ...
    mini->SetFixedVariable(idx++,"Pull_AmC",0);
    mini->SetFixedVariable(idx++,"Pull_eps_corr",0);
    for(int d = 0;d<8;d++){
        parname = TString::Format("Pull_eps_uncorr_%i",d);
        mini->SetFixedVariable(idx++,parname,0);
    }
    ...
    mini->SetFixedVariable(idx++,"Pull_protonDensity_GdLS",0.);
    mini->SetFixedVariable(idx++,"Pull_protonDensity_LS",0.);
    mini->SetFixedVariable(idx++,"Pull_sin2_theta12",sin2_theta12[0]);
    mini->SetFixedVariable(idx++,"Pull_delta_m2_21",delta_m2_21[0]);
    mini->SetFixedVariable(idx++,"Pull_delta_m2_32",delta_m2_32[0]);
    mini->SetVariable(idx++,"sin2_2theta13",0.086,0.0001);
    if(norm)
        mini->SetVariable(idx++,"NORM",0.95,0.0001);
    cout<<"Number of fit parameters = "<<idx<<"\n";
...
void parseAveragePower(const char* data){
    ...
    cout<<"PARSING AVERAGE POWER END\n";
}
